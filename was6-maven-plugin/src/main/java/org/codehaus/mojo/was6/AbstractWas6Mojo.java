    ...
    extends AbstractMojo
{
    ...
    /**
     * Some operations can often fail due to internal errors in the J2EE EJB deployment stage.
     * Aditionally, the WsAdmin mojo can equally fail due to connection issues.
     * These errors are intermittent and suprious. Simply reruning it will often fix the error (assuming there is
     * no real underlying error). Issues arise however, if this occurs in a release - the release will be
     * unnecessarilly aborted. This variable is designed to allow a number of retries to avoid this issue.
     * 
     * @since 1.2
     * @parameter expression="${was6.retryCount}" default-value=0
     */
       ...
       if(!isSkip()){
            ...
            while (attemptExecution)
            {
                try
                {
                    ...
                    // Execution worked, no need to try again.
                ...
                catch (MojoFailureException mfe)
                {
                    ...
                    if ( retryCount == 0)
                    {
                        // No retry count, or retry count has been exceeded.
                        throw mfe;
                    }
                    else
                    {
                        retryCount-- ;
                        getLog().error( "Build Failed. Retrying. Retry Count is now: " + retryCount );
                    }
                }
            }
        }else{
